console.log('Movies will be rendered in some seconds.');
...
(async function load() {
	...
	async function getData(url) {
		...
		if (data.data.movie_count > 0) {
			// https://platzi.com/clases/1316-jquery-js/12338-manejo-de-errores/ 3.50
			return data;
		}
		// if there are no movies, here it goes;
	...
	$form.addEventListener('submit', async (event) => {
		...
		try {
			// https://platzi.com/clases/1316-jquery-js/12338-manejo-de-errores/ 6.00
			const {
				data: { movies: pelis },
			} = await getData(
				`https://yts.mx/api/v2/list_movies.json?limit=1&query_term=${data.get(
					'name'
				)}`
			);
